...
graded(1, async () => {
  ...
  const own = Object.keys(schema.paths)
    ...
    .sort();
  expect(own).toEqual(['workshopCode', 'score', 'comment', 'submittedBy'].sort());

  const code = schema.path('workshopCode');
...
graded(2, async () => {
  ...
  expect(compoundUnique[0][0]).toEqual({ workshopCode: 1, submittedBy: 1 });
});

graded(3, async () => {
  const invalidValues = [0, 6, 3.5, 'abc'];
  for (const bad of invalidValues) {
    const res = await api.post(BASE, { workshopCode: 'WS101', score: bad });
  ...
  const missingValue = await api.post(BASE, { workshopCode: 'WS101' });
...
graded(5, async () => {
  ...
  const b = await seed({ workshopCode: 'WS202', score: 2 });
...
graded(9, async () => {
  ...
  await seed({ score: 4 });
  await seed({ score: 3 });
  await seed({ workshopCode: 'WS202', score: 1 });

  const spy = jest.spyOn(Feedback, 'aggregate');
  try {
    const res = await api.get(`${BASE}/summary?workshopCode=WS101`);
    expect(res.status).toBe(200);
    expect(res.body).toEqual({ workshopCode: 'WS101', averageScore: 4, feedbackCount: 3 });
    expect(spy).toHaveBeenCalled();

    const empty = await api.get(`${BASE}/summary?workshopCode=WS999`);
    expect(empty.status).toBe(200);
    expect(empty.body).toEqual({ workshopCode: 'WS999', averageScore: 0, feedbackCount: 0 });
  } finally {
    spy.mockRestore();
  }
});
...
graded(10, async () => {
  ...
  expect(noQuery.body).toEqual({ message: 'workshopCode is required' });
  ...
  const reachable = [
    ...
    await api.post(BASE, { workshopCode: 'WS202', score: 3, submittedBy: newId() }),
