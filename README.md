# Task 1.1: Workshop Feedback API

You are building the backend for a workshop feedback service, using Express and MongoDB (Mongoose) only, with no frontend. Anyone can browse and submit workshop feedback, and there is no login for this resource.
...
| field | type | rules |
|---|---|---|
| `workshopCode` | String | required (e.g. `"WS101"`) |
...
- Create: `workshopCode` required string; `score` required integer
...
| GET | `/api/feedback/summary?workshopCode=WS101` | `getFeedbackSummary` | `200` (see section 4) |
...
`GET /api/feedback/summary?workshopCode=WS101` returns:

```
{ "workshopCode": "WS101", "averageScore": <number>, "feedbackCount": <integer> }
```

- Compute it with `Feedback.aggregate()`: `$match` on `workshopCode`, then
  `$group` with `$avg` of `score` and `$sum: 1`. Loading documents
  with `find()` and averaging in JavaScript does not count.
- If nothing matches, return the requested `workshopCode` with
  `averageScore: 0` and `feedbackCount: 0`.
- If the `workshopCode` query parameter is missing, respond `400` with
